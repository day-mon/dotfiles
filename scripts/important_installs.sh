...
set -- topgrade bat exa go xsel feh picom flameshot docker xorg-xfd docker-compose zsh nemo neofetch kitty neovim discord cronie betterdiscordctl jetbrains-toolbox nvim-packer-git enquirer youtubemusic noto-fonts-cjk noto-fonts-emoji noto-fonts-extra
...
for i in "$@"; do
    if ! pacman -Qs "$i" > /dev/null 2>&1; then
        ...
        if ! sudo pacman -S "$i" --noconfirm > /dev/null 2>&1; then
           if ! paru -S "$i" --noconfirm > /dev/null 2>&1; then
               print_red "🚫 Failed to install $i"
           else
               print_green "📦 Instlled ${GREEN_UNDERLINE}$i${NC}"
           fi
        else
